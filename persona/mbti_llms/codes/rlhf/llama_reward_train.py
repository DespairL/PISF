...
from llama_dschat.utils.data.data_utils import create_prompt_dataset, DataCollatorReward
from llama_dschat.utils.utils import print_rank_0, to_device, save_hf_format, set_random_seed, get_all_reduce_mean, get_optimizer_grouped_parameters, save_zero_three_model, load_hf_tokenizer
from llama_dschat.utils.ds_utils import get_train_ds_config
from llama_dschat.utils.module.lora import convert_linear_layer_to_lora, convert_lora_to_linear_layer, only_optimize_lora_parameters, make_model_gradient_checkpointing_compatible
import logging
...
def main():
    ...
    def calculate_trainable_parameter_ratio(model):
        total_parameters = sum(p.numel() for p in model.parameters())
        trainable_parameters = count_parameters(model)
        ratio = trainable_parameters / total_parameters
        return ratio

    print_rank_0(f'Trainable Parameter Rate:{calculate_trainable_parameter_ratio(rm_model):.4f}%')

    train_phase = 2
    train_dataset, eval_dataset = create_prompt_dataset(
        args.local_rank, args.data_path, args.data_split,
        args.data_output_path, train_phase, args.seed, tokenizer,
        args.max_seq_len, args.model_base, reload=True, end_of_conversation_token=args.end_of_conversation_token)
    ...
    with open(args.file, 'w', encoding='UTF-8') as file_c:
        file_c.truncate(0)

    # Train!
    print_rank_0("***** Running training *****", args.global_rank, write_file=args.file)

    print_rank_0(
        f"***** Evaluating reward, Epoch {0}/{args.num_train_epochs} *****",
        args.global_rank, write_file=args.file)
    ...
    print_rank_0(
        ...
        f"acc (higher is better) : {acc}", args.global_rank, write_file=args.file)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), position=0):
            ...
            if step % 20 == 0:
                print_rank_0(f"Epoch {epoch+1}/{args.num_train_epochs}, Step {step} with loss {loss.item()}",
                    args.global_rank, write_file=args.file)
