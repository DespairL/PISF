...
def print_rank_0(msg, rank=None, write_file=None):
    if rank is not None and rank <= 0:
        if write_file:
            with open(write_file, 'a', encoding='UTF-8') as file_c:
                print(msg, file=file_c)
        print(msg)
    elif is_rank_0():
        if write_file:
            with open(write_file, 'a', encoding='UTF-8') as file_c:
...
def get_tokenizer(model_name_or_path, fast_tokenizer=True):
    if "Llama" in model_name_or_path:
        ...
        # chatglm tokenizer padding_side must be left, so we don't need to flip later
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_name_or_path, pad_token='<|extra_0|>',
            eos_token='<|endoftext|>', padding_side='right', trust_remote_code=True)
    else:
