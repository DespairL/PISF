...
def create_hf_model(model_class,
    ...
    if critical:
        # change it if rlhf training using other model
        model_path_mapping = {
        ...
        if 'chatglm' in model_name_or_path:
            ...
            model_key = 'llama'
        elif 'qwen' in model_name_or_path or 'Qwen' in model_name_or_path:
            model_key = 'qwen'
        else:
            raise NotImplementedError
        tuning_model_path = model_path_mapping[model_key]
        # Qwen Model
        ...
        if 'qwen' in model_name_or_path or 'Qwen' in model_name_or_path:
            model_config = AutoConfig.from_pretrained(tuning_model_path, trust_remote_code=trust_remote_code, pad_token_id=tokenizer.pad_token_id,
                                                    bf16=True)
        else:
            model_config = AutoConfig.from_pretrained(tuning_model_path, trust_remote_code=trust_remote_code)
    else:
        if 'qwen' in model_name_or_path or 'Qwen' in model_name_or_path:
            model_config = AutoConfig.from_pretrained(model_name_or_path, trust_remote_code=trust_remote_code, pad_token_id=tokenizer.pad_token_id,
                                                    bf16=True)
        else:
    ...
    empty_init = not (ds_config["zero_optimization"]["stage"] == 3)

    if rlhf_training:
        # the weight loading is handled by create critic model
        if 'chatglm' in model_name_or_path:
            model = model_class.from_config(model_config, trust_remote_code=trust_remote_code, empty_init=empty_init)
        elif 'qwen' in model_name_or_path or 'Qwen' in model_name_or_path:
        ...
        if 'chatglm' in model_name_or_path:
            model = model_class.from_pretrained(
                ...
                config=model_config, trust_remote_code=trust_remote_code, empty_init=empty_init)
        elif 'qwen' in model_name_or_path or 'Qwen' in model_name_or_path:
            if ds_config["zero_optimization"]["stage"] == 3:
                model = model_class.from_pretrained(
                    model_name_or_path, from_tf=bool(".ckpt" in model_name_or_path),
                    config=model_config, trust_remote_code=trust_remote_code)
            else:
                # use deepspeed-2
                model = model_class.from_pretrained(
                    model_name_or_path, from_tf=bool(".ckpt" in model_name_or_path),
                    config=model_config, trust_remote_code=trust_remote_code, device_map=f"cuda:{os.environ['LOCAL_RANK']}")
        else:
            model = model_class.from_pretrained(
                model_name_or_path,
                from_tf=bool(".ckpt" in model_name_or_path),
    ...
    if 'chatglm' not in model_name_or_path and "qwen" not in model_name_or_path and "Qwen" not in model_name_or_path:
...
def create_critic_model(model_name_or_path,
    ...
    start = time.time()
    critical = True if rlhf_training else False
    if 'Qwen' in model_name_or_path or 'qwen' in model_name_or_path:
        critic_model = create_hf_model(AutoModelForCausalLM, model_name_or_path, tokenizer,
                                    ds_config, rlhf_training, dropout, critical=critical)
    else:
