...
from ..utils.data.raw_datasets import induction_system_prompt
import pdb
...
class DeepSpeedPPOTrainer():
    ...
    def _generate_sequence(self, prompts, mask, step):
        ...
        if self.actor_model.module.config.model_type == "llama":
            # do_sample=False, do_sample=True all the time will both cause CUDA Error.
            # So we have to randomly change the generation schedule for each batch.
            # if it still raise cuda error, please resume training.
            kwargs = dict(do_sample=do_sample, temperature=temperature, top_p=top_p)
        elif self.actor_model.module.config.model_type == "chatglm":
            ...
            kwargs = dict(do_sample=do_sample, temperature=temperature, top_p=top_p, max_new_tokens=self.max_answer_seq_len, logits_processor=logits_processor,)
            # kwargs = dict(do_sample=False, max_new_tokens=self.max_answer_seq_len, logits_processor=logits_processor,)
        elif self.actor_model.module.config.model_type == "qwen":
            # qwen has default generation config
            from transformers import GenerationConfig
            # do_sample=do_sample, temperature=temperature, top_p=top_p,
            self.actor_model.module.generation_config = GenerationConfig.from_pretrained(self.args.actor_model_name_or_path,
        ...
        if self.actor_model.module.config.model_type == "qwen":
            print_rank_0(f'kwargs:{self.actor_model.module.generation_config}')
        else:
            print_rank_0(f'kwargs:{kwargs}')

        with torch.no_grad():
            if self.actor_model.module.config.model_type == "chatglm":
                ...
                #token_type_ids = self.tokenizer(self.tokenizer.batch_decode(prompts, skip_special_tokens=True),
                #                            return_tensors='pt', padding='max_length', max_length=prompts.shape[1])['token_type_ids']
                # inputs = {'input_ids':prompts, 'attention_mask':mask, 'token_type_ids':token_type_ids}
                seq = self.actor_model.module.generate(
                    prompts,
                    attention_mask=mask,
                    # token_type_ids=token_type_ids,
                    return_dict_in_generate=False,
                    generation_config=self.actor_model.module.generation_config,
                    synced_gpus=self.z3_enabled)
            else:
                seq = self.actor_model.module.generate(
                    prompts,
        ...
        if self.args.print_answers and (step % self.args.print_answers_interval
                                        == 0):
            print_batch_size = batch_size // 4 if batch_size > 4 else 2
            print_rank_0(
                f"--- prompt --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.batch_decode(prompts[:print_batch_size, :], skip_special_tokens=True)}"
            )
            print_rank_0(
                f"--- ans    --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.batch_decode(ans[:print_batch_size, :], skip_special_tokens=True)}"
            )
        ...
        return out_seq, prompts, ans

    def generate_experience(self, prompts, mask, step):
        self.eval()
        generate_start = time.time()
        seq, prompts, ans = self._generate_sequence(prompts, mask, step)
        ...
        #if self.actor_model.module.config.model_type == "chatglm":
        #    # replace eos token 2 to added_special_token
        #    bs = seq.shape[0]
        #    seq_len = seq.shape[1]
        #    for i in range(bs):
        #        c_inds = (seq[i][self.prompt_length:] == self.tokenizer.pad_token_id).nonzero() # padding of answer
        #        # here we only use the answer part of the sequence so we do not need to care about the padding at the beginning
        #        c_ind = c_inds[0].item() + self.prompt_length if len(
        #            c_inds) > 0 else seq_len
        #        c_ind = min(c_ind, seq_len)
        #        seq[i][c_ind-1] = self.end_of_conversation_token_id
    ...
    def train_rlhf(self, inputs, pretrain_critic):
