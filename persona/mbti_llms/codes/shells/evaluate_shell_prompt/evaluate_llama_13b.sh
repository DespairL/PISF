...
model="Llama-2-13b"
model_path=/data/NJU/datasets/persona/models/${model}-chat-hf/
...
output_path="/data/NJU/datasets/persona/mbti_llms/results/debias_results/llama_results/${model}_results/"
mkdir -p $output_path
GPU=1
...
do
    ...
    do
        ...
        CUDA_VISIBLE_DEVICES=$GPU torchrun --nnodes=1 --nproc-per-node=1 --rdzv-backend=c10d \
            ...
            /data/NJU/datasets/persona/mbti_llms/codes/prompt_test/evaluate_all_llama.py \
