set -e
GPU=7
write_path_c="/data/NJU/datasets/persona/mbti_llms/results/debias_results/cp_qwen.txt"
...
output_path="cp_qwen.json"
for mbti_type in E I S N T F J P #ESTP ESTJ ESFP ESFJ ENFJ ENFP ENTP ENTJ ISTJ ISTP ISFP ISFJ INFJ INFP INTP INTJ # #no
do
    ...
    if [ "$mbti_type" = "no" ]; then
        ...
        model="/data/NJU/datasets/persona/models/continual_pretrain/Qwen-7B-Chat-${mbti_type}/${checkpoint}"
    fi
    for i in 0 1 2 3 4
    do
